...
├── config.yaml          # [УСТАРЕВШИЙ] Старая конфигурация (не используется)
├── gpt2giga.log         # Логи приложения (создаётся автоматически)
└── README.md            # Этот файл
```
...
Спасибо [Андрею](https://habr.com/ru/articles/972852/) за подробную статью, на основе которой было подготовлено данное решение. Статья оказалась очень полезной при разработке данного решения.
